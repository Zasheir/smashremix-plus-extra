...
scope Command {
    ...
    scope end_hitbox_: {
        ...
        addiu   t2, r0, -0x0001         // t2 = -1 (0xFFFFFFFF)
        sw      t2, 0x0000(t0)          // disable fgm override for hitboxes 1 and 2
        sw      t2, 0x0004(t0)          // disable fgm override for hitboxes 3 and 4

        li      t0, set_hitbox_hitlag_mul.player_table
        addu    t0, t0, t1              // t0 = hitbox_hitlag_mul table + (port * 4))
        lw      t0, 0x0000(t0)          // t0 = px_hitbox_hitlag_mul
        addiu   t2, r0, -0x0001         // t2 = -1 (0xFFFFFFFF)
        sw      t2, 0x0000(t0)          // disable hitlag multiplier override for hitboxes 1 and 2
        sw      t2, 0x0004(t0)          // disable hitlag multiplier override for hitboxes 3 and 4

        li      t0, set_hitbox_di_mul.player_table
        addu    t0, t0, t1              // t0 = hitbox_di_mul table + (port * 4))
        lw      t0, 0x0000(t0)          // t0 = px_hitbox_di_mul
        addiu   t2, r0, -0x0001         // t2 = -1 (0xFFFFFFFF)
        sw      t2, 0x0000(t0)          // disable di multiplier override for hitboxes 1 and 2
        sw      t2, 0x0004(t0)          // disable di multiplier override for hitboxes 3 and 4

        lw      t0, 0x0004(sp)          // ~
        lw      t1, 0x0008(sp)          // ~
        lw      t2, 0x000C(sp)          // load t0, t1, t2
    ...
    scope create_hitbox_: {
        ...
        lbu     t1, 0x000D(s1)          // t1 = player port
        sll     t1, t1, 0x2             // ~
        addu    t0, t0, t1              // t0 = set_hitbox_hitlag_mul table + (port * 4))
        lw      t0, 0x0000(t0)          // t0 = px_hitbox_hitlag_mul
        sll     t1, a1, 0x1             // ~
        addu    t0, t0, t1              // t0 = px_hitbox_hitlag_mul + (hitbox id * 2)
        addiu   t1, r0, -0x0001         // t1 = -1 (0xFFFFFFFF)
        sh      t1, 0x0000(t0)          // reset hitlag override for current hitbox

        li      t0, set_hitbox_di_mul.player_table
        lbu     t1, 0x000D(s1)          // t1 = player port
        sll     t1, t1, 0x2             // ~
        addu    t0, t0, t1              // t0 = set_hitbox_di_mul table + (port * 4))
        lw      t0, 0x0000(t0)          // t0 = px_hitbox_di_mul
        sll     t1, a1, 0x1             // ~
        addu    t0, t0, t1              // t0 = px_hitbox_di_mul + (hitbox id * 2)
        addiu   t1, r0, -0x0001         // t1 = -1 (0xFFFFFFFF)
        sh      t1, 0x0000(t0)          // reset di override for current hitbox
    ...
    // ZZZZ = hitlag multiplier (upper float)
    scope set_hitbox_hitlag_mul: {
        constant COMMAND_LENGTH(0x4)
        li      t0, player_table
        lbu     t1, 0x000D(s2)              // t1 = player port
        sll     t1, t1, 0x2                 // ~
        addu    t0, t0, t1                  // a0 = hitlag_table + (port * 4))
        lw      t0, 0x0000(t0)              // t0 = px_hitbox_hitlag_multi
        lh      t1, 0x0002(v0)              // t1 = mul
        ...
        sll     t2, t2, 0x1                 // ~
        addu    t0, t0, t2                  // t2 = hitbox_hitlag
        sh      t1, 0x0000(t0)              // store mul
        b       _end
        nop

        _set_for_all:
        sh      t1, 0x0000(t0)              // store mul for hitbox 1
        sh      t1, 0x0002(t0)              // store mul for hitbox 2
        sh      t1, 0x0004(t0)              // store mul for hitbox 3
        sh      t1, 0x0006(t0)              // store mul for hitbox 4
        ...
        // 800E3EBC+600
        scope hitlag_attacker_multiply: {
            OS.patch_start(0x5FCBC, 0x800E44BC)
            j hitlag_attacker_multiply
            sw t6, 0x800(s5) // original line 2
            _return:
            OS.patch_end()

            // s1 = attacker player struct
            // s2 = hitlog

            _multiplier:
            li t1, player_table // t1 = table base address
            lbu t0, 0xD(s1) // t0 = player port
            sll t0, t0, 0x2 // t0 = player port * 4
            addu t1, t1, t0 // t1 = player base address
            lw t1, 0x0(t1) // t1 = player base
            lw t0, 0x4(s2) // t0 = pointer to hitbox object (FTAttackColl)
            addiu at, s1, 0x294 // checking hitbox 0 address
            beq t0, at, _continue // addresses match
            addiu at, r0, 0x0 // t0 = hitbox id (0)
            addiu at, s1, 0x358 // checking hitbox 1 address
            beq t0, at, _continue // addresses match
            addiu at, r0, 0x1 // t0 = hitbox id (1)
            addiu at, s1, 0x41C // checking hitbox 2 address
            beq t0, at, _continue // addresses match
            addiu at, r0, 0x2 // t0 = hitbox id (2)
            addiu at, s1, 0x4E0 // checking hitbox 3 address
            beq t0, at, _continue // addresses match
            addiu at, r0, 0x3 // t0 = hitbox id (3)
            _continue:
            sll t0, at, 0x1 // t0 = hitbox id * 2
            addu t1, t1, t0 // t1 = hitbox address
            lh t0, 0x0(t1) // t0 = multiplier for this hitbox

            andi at, t0, 0x8000 // check if it's a negative value
            bnez at, _original // if so, skip
            nop
            
            _multiplier_continue:
            sll t0, t0, 16 // t0 = upper(t0) (move value to upper XXXX0000)
            b _b_960 // skip checking and setting electric multiplier
            sw t0, 0x7A4(s1) // save new hitlag multiplier value (attacker_fp->hitlag_mul)

            _original:
            lli at, 0x2 // original line 0: at = electric damage type
            bne v0, at, _b_960 // original line 1
            nop

            _end:
            j _return // goes into applying electric damage multiplier
            nop
        ...
        // 800E3EBC + 968
        scope hitlag_defender_multiply: {
            OS.patch_start(0x60024, 0x800E4824)
            j hitlag_defender_multiply
            swc1 f16, 0x7E0(s5) // original line 1
            _return:
            OS.patch_end()

            // s2 = hitlog
            // s5 = defender player struct

            lw v1, 0(s2) // v1 = hitlog->hit_source

            // reset DI multiplier for this character
            li t0, DI.player_di_multiplier
            lbu t1, 0xD(s5) // t1 = player port
            sll t1, t1, 0x1 // t1 = player port * 2
            addu t0, t0, t1 // t0 = player DI multiplier address
            addiu at, r0, -1
            sh at, 0x0(t0) // DI multiplier = -1

            // switch(hitlog->attacker_object_class)
            lli at, 0x1 // nFTHitLogObjectFighter (direct hit)
            beq v1, at, _direct_hit
            lli at, 0x2 // nFTHitLogObjectWeapon (projectile)
            beq v1, at, _projectile_hit
            nop
            b _original
            nop
            ...
            scope _direct_hit: {
                ...
                lw t0, 0x4(s2) // t0 = pointer to hitbox object (FTAttackColl)
                addiu at, s1, 0x294 // checking hitbox 0 address
                beq t0, at, _hitbox_found // addresses match
                addiu at, r0, 0x0 // t0 = hitbox id (0)
                addiu at, s1, 0x358 // checking hitbox 1 address
                beq t0, at, _hitbox_found // addresses match
                addiu at, r0, 0x1 // t0 = hitbox id (1)
                ...
                addiu at, r0, 0x3 // t0 = hitbox id (3)
                _hitbox_found:
                sll t2, at, 0x1 // t2 = hitbox id * 2

                scope di: {
                    li t1, Command.set_hitbox_di_mul.player_table // t1 = table base address
                    lbu t0, 0xD(s1) // t0 = attacker player port
                    sll t0, t0, 0x2 // t0 = attacker player port * 4
                    addu t1, t1, t0 // t1 = attacker player base address
                    lw t1, 0x0(t1) // t1 = attacker player base

                    addu t1, t1, t2 // t1 = hitbox address
                    lh t0, 0x0(t1) // t0 = multiplier for this hitbox
                    ...
                    li t1, DI.player_di_multiplier
                    lbu at, 0xD(s5) // at = player port
                    sll at, at, 0x1 // at = player port * 2
                    addu t1, t1, at // t1 = player di multiplier address
                    sh t0, 0x0(t1) // save di multiplier for this player slot

                    _end:
                }

                scope hitlag: {
                    li t1, Command.set_hitbox_hitlag_mul.player_table // t1 = table base address
                    lbu t0, 0xD(s1) // t0 = attacker player port
                    sll t0, t0, 0x2 // t0 = attacker player port * 4
                    addu t1, t1, t0 // t1 = attacker player base address
                    lw t1, 0x0(t1) // t1 = attacker player base
                    ...
                    sll t0, t0, 16 // t0 = upper(t0) (move value to upper XXXX0000)
                    b _b_980 // skip checking and setting electric multiplier
                    sw t0, 0x7A4(s5) // save new hitlag multiplier value (attacker_fp->hitlag_mul)
                }
            }

            scope _projectile_hit: {
                // s4 = projectile struct
                lw t0, 0xC(s4) // t0 = projectile ID

                addiu at, r0, Projectile.VANILLA_LAST_ID
                blt t0, at, _original // it's a vanilla projectile, skip
                nop

                // if ID is greater than the max allowed number,
                // this projectile probably isn't using the Projectile.add_projectile
                // skip this one
                addiu at, r0, Projectile.MAX_REMIX_PROJECTILE_NUM
                bgt t0, at, _original
                nop

                subiu t0, t0, Projectile.VANILLA_LAST_ID // start counting from Remix projectile IDs
                sll t2, t0, 0x1 // t2 = projectile ID * 2

                scope di: {
                    li t1, Projectile.di_multiplier.table
                    addu t1, t1, t2 // address for this projectile ID
                    lh t0, 0x0(t1) // t0 = multiplier for this projectile ID
                ...
                scope hitlag: {
                    li t1, Projectile.hitlag_multiplier.table
                    addu t1, t1, t2 // address for this projectile ID
                    lh t0, 0x0(t1) // t0 = multiplier for this projectile ID
            ...
            _original:
            lli at, 0x2 // original line 0: at = electric damage type
            bne v0, at, _b_980 // original line 1
            nop
    ...
    // Y    = hitbox id (0-3 = hitbox 1-4)
    // ZZZZ = hitlag multiplier (upper float)
    scope set_hitbox_di_mul: {
        constant COMMAND_LENGTH(0x4)
        li      t0, player_table
        lbu     t1, 0x000D(s2)              // t1 = player port
        sll     t1, t1, 0x2                 // ~
        addu    t0, t0, t1                  // a0 = di_table + (port * 4))
        lw      t0, 0x0000(t0)              // t0 = px_hitbox_di_multi
        lh      t1, 0x0002(v0)              // t1 = mul
        lbu     t2, 0x0001(v0)              // t2 = hitbox flags
        srl     t3, t2, 0x4                 // t3 = X
        bnez    t3, _set_for_all            // if flagged to apply to all hitboxes, then do so
    ...
    dw      L_voice_sfx                     // 0xDC
    dw      set_hitbox_hitlag_mul           // 0xDD
    dw      set_hitbox_di_mul               // 0xDE
    dw      OS.NULL                         // 0xDF
    dw      OS.NULL                         // 0xE0
    dw      OS.NULL                         // 0xE1
    dw      OS.NULL                         // 0xE2
