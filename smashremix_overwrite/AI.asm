...
scope AI {
    ...
    scope level_ten: {
        ...
        // 801346D4+8
        // This function makes the CPU jump when offstage based on if they're facing
        // the ledge or not. For lv10, we can just skip this function completely
        scope fix_offstage_early_jump: {
            OS.patch_start(0xAF11C, 0x801346DC)
            j fix_offstage_early_jump
            addiu t6, sp, 0x10 // original line 1
            _return:
            OS.patch_end()
            ...
            // Check CPU level
            lbu t1, 0x13(a0) // t1 = cpu level
            addiu t1, t1, -10 // t1 = 0 if level 10
            bnezl t1, _original // if not lv10, skip
            nop

            // if lv10
            j 0x801346D4+0x288 // jump to the end of the function
            nop

            _original:
            j _return
            lw t9,0x1C(t7) // original line 2
        }
        ...
        scope improved_recovery: {
            ...
            scope _find_closest_ledge: {
                ...
                scope _loop: { // loop inner logic
                    ...
                    _check_ground_below: {
                        ...
                        // create a vec3 at 0x20(sp)
                        // make it just below ledge pos
                        swc1 f4, 0x20(sp) // x
                        
                        lui at, 0x3F80 // at = 1.0
                        mtc1 at, f20 // f20 = 1.0
                        sub.s f20, f6, f20 // f20 = ledge.y - 1.0
                        swc1 f20, 0x24(sp) // y

                        sw r0, 0x28(sp) // z

                        addiu a0, sp, 0x20 // a0 = ledge pos vec3
                        or a1, r0, r0 // a1 pointer = null (we're not using the results the function brings)
                        or a2, r0, r0 // a2 pointer = null (we're not using the results the function brings)
                        or a3, r0, r0 // a3 pointer = null (we're not using the results the function brings)
                        jal 0x800F9348 // mpCollisionCheckProjectFloor(Vec3f *position, s32 *project_line_id, f32 *ga_dist, u32 *stand_coll_flags, Vec3f *angle)
                        sw r0, 0x10(sp) // arg4 = pointer = null (we're not using the results the function brings)
                    ...
                    sub.s f10, f10, f6 // f10 = y distance
            ...
            scope _improve_recovery_drift: {
                ...
                scope _update_left_ledge_pos: {
                    ...
                    // For a1 (location Y) we offset our Y based on
                    // if we already used our doublejump and we're moving downwards
                    lwc1 f2, 0x4(t0) // f2 = location Y
                    lui at, 0x447A // load constant 1000.0
                    mtc1 at, f4 // f4 = constant (for down)
                    lwc1 f6, 0x01CC+0x90(a0) // f6 = com->jump_predict (for up)

                    lli t1, 0x2 // always consider max jumps to be 2 because of jump height falloff
                    lb at, 0x148(a0) // jumps used
                    bltl at, t1, _set_location_y // if jumps used < 2, continue normally
                    add.s f2, f2, f6 // ...and increase Y to use our doublejump height
                    lwc1 f8, 0x4C(a0) // f8 = air Y velocity
                    mtc1 r0, f0 // f0 = 0
                    c.lt.s f0, f8 // 0 < yspeed?
                    nop
                    bc1tl _set_location_y // if 0 < yspeed, continue normally
                    add.s f2, f2, f6 // ...and increase Y to use our doublejump height

                    sub.s f2, f2, f4 // otherwise, decrease Y location to prefer lower ledges
                    _set_location_y:
                    mfc1 a1, f2 // a1 = location Y
                    ...
                    sh v1, 0x01CC+0x44(a0) // save if nearest LEFT ledge is grabbable (using unused space in FTComputer)
                }

                scope _update_right_ledge_pos: {
                    ...
                    addiu   sp, sp, 0x30 // deallocate memory
                    // set right ledge pos
                    lw at, 0x0(v0) // at = right ledge pos x
                    sw at, 0x01CC+0x54(a0) // save nearest RIGHT ledge X
                    lw at, 0x4(v0) // at = right ledge pos y
                    sw at, 0x01CC+0x58(a0) // save nearest RIGHT ledge Y
                    sw v1, {ledge_grabbable_right} // save ledge grabbable
                    sh v1, 0x01CC+0x46(a0) // save if nearest RIGHT ledge is grabbable (using unused space in FTComputer)
                }
                ...
                // this makes CPUs less scared of crossing big gaps in the stage
                // location Y is in f4
                lw      t1, 0x9C8(a0) // t1 = attribute pointer
                lw      t1, 0x064(t1) // t1 = max jumps
                lb      at, 0x148(a0) // jumps used
                beql    at, t1, pc()+0xC // if no jumps left, set lower threshold
                or      at, r0, r0 // constant = 0
                ...
                nop
                bc1tl   _b_ccc // skip recovery completely
                addiu   sp, sp, 0x30
                ...
                lli t1, 0x2 // always consider max jumps to be 2 because of jump height falloff
                lb at, 0x148(a0) // jumps used
                blt at, t1, _decide_ledge // if jumps used < 2, continue normally
                nop
                // if here, we have have used a double jump
                // and we're moving downwards.
                // prefer lower ledges
                sub.s f4, f4, f6

                _decide_ledge:
                // distance to right ledge
                lwc1 f6, 0x01CC+0x54(a0) // nearest RIGHT ledge X
                sub.s f8, f2, f6 // f8 = X distance to RIGHT ledge X
                lwc1 f6, 0x01CC+0x58(a0) // nearest RIGHT ledge Y
                sub.s f10, f4, f6 // f10 = Y distance to RIGHT ledge Y

                mul.s f12, f8, f8 // f12 = (x distance)^2
                mul.s f14, f10, f10 // f14 = (y distance)^2
                add.s f12, f12, f14 // f12 = (x distance)^2 + (y distance)^2
                sqrt.s f16, f12 // f16 = sqrt((x distance)^2 + (y distance)^2) = distance to right ledge

                // distance to left ledge
                lwc1 f6, 0x01CC+0x4C(a0) // nearest LEFT ledge X
                sub.s f8, f2, f6 // f8 = X distance to LEFT ledge X
                lwc1 f6, 0x01CC+0x50(a0) // nearest LEFT ledge Y
                sub.s f10, f4, f6 // f10 = Y distance to LEFT ledge Y

                mul.s f12, f8, f8 // f12 = (x distance)^2
                mul.s f14, f10, f10 // f14 = (y distance)^2
                add.s f12, f12, f14 // f12 = (x distance)^2 + (y distance)^2
                sqrt.s f18, f12 // f18 = sqrt((x distance)^2 + (y distance)^2) = distance to left ledge

                c.lt.s f16, f18 // distance to left ledge < distance to right ledge?
                nop
                ...
                scope below_ledge: {
                    ...
                    lw at, {ledge_grabbable} // if ledge is not grabbable, do not add outwards padding
                    beqz at, _end
                    nop
                ...
                // here we also make a change to the original:
                // if we have jumps remaining, drop a little lower before using our double jump
                // check against 2 jumps only since most characters with many jumps have a huge height falloff
                lli t1, 0x2 // always consider max jumps to be 2
                lb at, 0x148(a0) // jumps used
                bge at, t1, _modified_original // if jumps used >= 2, go with original
                nop
                // If here, we have jumps to use. So we can be less desperate and fall a bit more
                lw t8, 0x8e8(s0)
                lwc1 f14, 0x20(t8)
                lw t0, 0x44(sp)
                lwc1 f16, 0x64(t0) // f16 = target y
                lwc1 f2, 0x01CC+0x90(a0) // com->jump_predict
                lui at, 0x4348 // at = 200.0
                mtc1 at, f4 // f4 = 200.0
                sub.s f2, f2, f4 // f2 = com->jump_predict - 200.0
                b _modified_original_continue
                sub.s f16, f16, f2 // consider our doublejump height before deciding we're too low
                ...
                lw t0, 0x44(sp)
                lwc1 f16, 0x64(t0) // f16 = target y
                _modified_original_continue:
                lwc1 f2, 0x4C(s0) // f2 = air Y velocity
                mtc1 r0, f0 // f0 = 0
                c.lt.s f0, f2 // if 0 < y speed
                nop
                bc1tl _b_ccc // if not moving down, don't jump or upB
                nop
                c.lt.s f14, f16 // just like the regular function, check for Y being below target
                j   _return
                nop
                ...
                _b_ccc:
                // Skips everything, returns from function
                j 0x80134E98+0xCCC
                nop
            }
        }
        ...
        scope shorthop_followup_attack: {
            ...
            lwc1    f2, 0x58(t1) // f2 = vel_damage_air.y
            add.s   f4, f4, f2 // factor in 1 frame
        ...
        scope fix_movement_prediction: {
            ...
            scope x: {
                ...
                scope predict_other_x: {
                    ...
                    damage_air: {
                        // here we prepare f0 to have the value we decrease
                        // from vel_damage_air.x every iteration frame.
                        lw t0, 0x1b0(sp) // t0 = other struct

                        addiu sp, sp, -0x20
                        swc1 f18, 0x18(sp) // save f18 = our predicted x
                        swc1 f8, 0x1C(sp) // save f8 = their predicted x

                        // f0 = damage_angle = atan2f(vel_damage_air->x, vel_damage_air->x)
                        lwc1 f12, 0x58(t0)
                        jal 0x8001863C // atan2f(f12, f14), result ends in f0
                        lwc1 f14, 0x54(t0)

                        // f0 = cosf(damage_angle)
                        mov.s f12, f0 // f12 = damage angle
                        jal 0x80035CD0 // f0 = cos(f12)(damage_angle)
                        nop

                        li at, 0x3FD9999A  // at = 1.7F
                        mtc1 at, f18 // f18 = 1.7F

                        mul.s f0, f18, f0 // f0 = (1.7F * cosf(damage_angle))

                        lwc1 f18, 0x18(sp) // restore f18 = our predicted x
                        lwc1 f8, 0x1C(sp) // restore f8 = their predicted x
                        addiu sp, sp, 0x20

                        lw t0, 0x1b0(sp) // t0 = other struct
                        lwc1 f14, 0x54(t0) // f14 = vel_damage_air.x
                        or at, r0, a0 // at = hit_frame (int)
                        _loop_start: {
                            // if at <= 0, end loop
                            blez at, _loop_end
                            nop
                            add.s f8, f8, f14 // f8 = target_pos_x += vel_damage_air.x
                            scope _vel_damage_air_iter: {
                                sub.s f14, f14, f0 // vel_damage_air->x -= (1.7F * cosf(damage_angle))
                                abs.s f24, f0 // f24 = (1.7F * cosf(damage_angle))
                                abs.s f26, f14 // f26 = vel_damage_air->x
                                c.le.s f26, f24 // if abs(vel_damage_air.x) < abs(subtractor), we have crossed over
                                nop
                                bc1tl _loop_end // if we crossed over, exit loop
                                nop
                                _end:
                            }
                            _continue_loop:
            ...
            scope y: {
                scope predict_my_y: {
                    ...
                    lw at, 0x14c(s0)
                    beqz at, _end
                    lwc1 f2, 0x1a8(sp) // f2 = this_pos_y = current y

                    // calculate predicted Y based on gravity and velocity
                    or at, r0, a0 // at = hit_frame (int)
                    lwc1 f2, 0x1a8(sp) // f2 = this_pos_y
                    ...
                    _loop_start: {
                        ...
                        blez at, _loop_end
                        nop
                        add.s f2, f2, f16 // f2 = this_pos_y += this_vel_y
                        ...
                        nop
                        bc1tl _loop_end
                        mov.s f2, f18 // final pos y = ground y
                        _ground_check_end:
                ...
                scope predict_other_y: {
                    ...
                    lw t0, 0x1b0(sp) // t0 = other struct
                    lw at, 0x14c(t0)
                    beqz at, _end // if grounded, skip
                    lwc1 f4, 0x190(sp) // f4 = target_pos_y = current y

                    damage_air_prep: {
                        // here we prepare f0 to have the value we decrease
                        // from vel_damage_air.y every iteration frame.
                        addiu sp, sp, -0x20
                        swc1 f2, 0x18(sp) // save f2 = our predicted y

                        // f0 = damage_angle = atan2f(vel_damage_air->y, vel_damage_air->x)
                        lwc1 f12, 0x58(t0)
                        jal 0x8001863C // atan2f(f12, f14), result ends in f0
                        lwc1 f14, 0x54(t0)

                        // f0 = __sinf(damage_angle)
                        mov.s f12, f0 // f12 = damage angle
                        ...
                        mul.s f0, f18, f0 // f0 = (1.7F * __sinf(damage_angle))

                        lwc1 f2, 0x18(sp) // restore f2 = our predicted y
                        addiu sp, sp, 0x20

                        lw t0, 0x1b0(sp) // restore t0 = other struct
                    }
                    ...
                    lwc1 f14, 0x58(t0) // f14 = vel_damage_air.y
                    ...
                    _loop_start: {
                        ...
                        add.s f4, f4, f14 // f4 = target_pos_y += vel_damage_air.y
                        sub.s f8, f8, f16 // f8 = distance to ground -= target_vel_y (this distance is negative)
                        sub.s f8, f8, f14 // f8 = distance to ground -= vel_damage_air.y (this distance is negative)
                        ...
                        scope _vel_damage_air_iter: {
                            // vel_damage_air.y -= (1.7F * __sinf(damage_angle)) we calculated earlier
                            sub.s f14, f14, f0
                            abs.s f24, f0 // f24 = abs(sinf(damage_angle))
                            abs.s f26, f14 // f26 = abs(damage_air_y)
                            c.le.s f26, f24 // if abs(vel_damage_air.y) < abs(subtractor), we have crossed over
                            nop
                            bc1tl _end // if we crossed over...
                            mov.s f14, f10 // ...clamp vel_damage_air.y to 0
                            _end:
                        }
                ...
                sub.s f6, f4, f2 // f6 = predict_pos_y
                swc1 f6, 0x16C(sp)
            }
