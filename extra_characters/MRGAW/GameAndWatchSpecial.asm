...
scope GameAndWatchScale {
    ...
    scope adjust_scale_for_direction_: {
        ...
        lli     t2, Character.id.MRGAW      // t2 = id.MRGAW
...
scope GameAndWatchNSP {
    ...
    // main subroutine for MRGAW's Chef
    scope main: {
        addiu   sp, sp, -0x0040
        sw      ra, 0x0014(sp)
        swc1    f6, 0x003C(sp)
	...
	sw    at, 0x0130(v1)
        ...
        lli   at, 0x0050		// make projectile knockback angle 80 degrees
	sw    at, 0x012C(v1)
	...
	mtc1	at, f6
        ...
        lw      t0, 0x0008(v0)
        addiu   t7, r0, Character.id.MRGAW
        bnel    t0, t7, _standard
        lui     t7, 0x3F80          // load normal reflect multiplier if not MRGAW and thereby top speed of MRGAW projectile will not increase
        ...
        dw 0x00000000
        dw Character.MRGAW_file_6_ptr     // pointer to file
	...
	float32 0.125                   // 0x0014 - rotation speed
        ...
        dw Character.MRGAW_file_6_ptr     // 0x0024   projectile data pointer
    ...
    scope air_to_ground_: {
        ...
        lli     a1, Kirby.Action.WOLF_NSP_Ground  //MRGAW_NSP_Ground
...
scope MRGAWSpecialsRefresh: {
    sw  r0, 0x0ADC(a0)              // set up special bool to FALSE
    jr  ra
    nop
}

Character.table_patch_start(on_hit, Character.id.MRGAW, 0x4)
dw MRGAWSpecialsRefresh;
OS.patch_end()

scope GameAndWatchUSP {
    // floating point constants for physics and fsm
    constant AIR_Y_SPEED(0x4358)            // current setting - float32 216
    constant GROUND_Y_SPEED(0x4358)         // current setting - float32 216
    constant X_SPEED(0x3e40)                // current setting - float32 0.1875 (was 0x3e00 or 0.125)
    constant X_SPEED_INIT(0x3f10)           // current setting - float32 0.5625
    constant GRAV_ACCELERATION(0x40e0)      // current setting - float32 7
    constant END_AIR_ACCELERATION(0x3f00)   // current setting - float32 0.5 (used to be 0x3C20 or 0.00977, 0.5 or 0x3f00)
    constant END_AIR_SPEED(0x41C0)          // current setting - float32 24
    constant LANDING_FSM(0x3E80)            // current setting - float32 0.25 (was 0.375 or 0x3EC0)
    ...
    // @ Description
    // Main subroutine for MRGAW's aerial up special.
    scope main_air_: {
    ...
    // @ Description
    // Subroutine which allows a direction change for MRGAW's up special.
	...
	// comments: this will call calc_velocity to determine direction based on joystick direction
        _check_begin_move:
        lw      t0, 0x0184(s0)              // t0 = temp variable 3
	...
	// comments: only runs once to start moving upward
        _calculate_velocity:
...
scope GameAndWatchDSP {
	...
	ori	a1, 0,  0						// reset a1
	addiu   a1, a1, 12						// set a1 to the damage given to MRGAW (currently 12)
	...
	lw      a0, 0x08E8(v0)              // v0 = MRGAW joint (he doesn't have a 0x95C joint?)
	lw      t0, 0x0084(a2)
