...
scope GameAndWatchNSP {
 // @ Description
    // main subroutine for MRGAWTHREED's Chef
    scope main: {
		...
		lui     t0, 0x4300              			
		mtc1	t0, f0								// load 128.0(fp) into f2
        ...
        nop                                 // ~
        bc1t    _decrease_angle             // branch to decrease angle (makes projectile go more vertical) stick_x * direction > 0
        nop
		...
		//lw	   t4, 0x002C(sp) // load projectile struct (0x0084(a0))
		//lw	   t5, 0x014C(t4) // load some value in the struct?
		//bnel   t5, at, _test_label
		//lwc1   f12, 0x0018(s0)
		//beq	 r0, r0, _test_label
		//lwc1   f12, 0x001c(s0)
		//lwc1   f12, 0x0018(s0)
		//_test_label:
		//swc1   f4, 0x0028(v1)
        ...
        //bc1f    _scaling
        //swc1    f6, 0x0024(a0)      // if speed is greater than max rightward velocity, save max speed
        //neg.s   f6, f6
        //c.le.s  f8, f6
        //nop
        //bc1t    _scaling
        //swc1    f6, 0x0024(a0)      // if speed is less than max leftward velocity, save max speed
		...
		// leftover from wolf's code (not using atm)
        //_scaling:
        ...
        lw      t0, 0x0008(v0)
        addiu   t7, r0, Character.id.MRGAWTHREED
        bnel    t0, t7, _standard
        lui     t7, 0x3F80          // load normal reflect multiplier if not MRGAWTHREED and thereby top speed of MRGAWTHREED projectile will not increase
        ...
        dw 0x00000000
        dw Character.MRGAWTHREED_file_6_ptr     // pointer to file
        ...
        dw Character.MRGAWTHREED_file_6_ptr     // 0x0024   projectile data pointer
   ...
   // Subroutine which handles air collision for neutral special actions
    ...
    //    nop
			...
			_check_cancel:
			// check if the current animation frame is higher than 14, if so, G&W is cooking
    ...
    scope air_to_ground_: {
        ...
        lli     a1, Kirby.Action.WOLF_NSP_Ground  //MRGAWTHREED_NSP_Ground
...
scope MRGAWTHREEDSpecialsRefresh: {
    sw  r0, 0x0ADC(a0)              // set up special bool to FALSE
    jr  ra
    nop
}

Character.table_patch_start(on_hit, Character.id.MRGAWTHREED, 0x4)
dw MRGAWTHREEDSpecialsRefresh;
OS.patch_end()
...
scope GameAndWatchUSP {
    ...
    constant LANDING_FSM(0x3F00)            // current setting - float32 0.25
    // parachute constants
    constant FLOAT_GRAVITY(0x3F00)          // current setting - float32 0.5
    constant FLOAT_FALL_SPEED(0x4170)       // current setting - float32 15
	...
	lw	t1, 0x0184(v0)	
        ...
        lli     a1, GameAndWatch.Action.Fire          // a1(action id) = USP
    ...
    // @ Description
    // Main subroutine for MRGAWTHREED's aerial up special.
    scope main_air_: {
    ...
    // @ Description
    // Subroutine which allows a direction change for MRGAWTHREED's up special.
    ...
    // @ Description
    // Subroutine which handles movement for Game and Watch's up special.
	...
	// comments: this only runs once, so gravity shouldn't affect what happens here
        _calculate_velocity:
        ...
        lui     t0, 0x3f00                  // ~
        mtc1    t0, f2                      // f2 = 0.5, used to be 4000 or 2
        mul.s   f2, f0, f2                  // f2 = x velocity (stick_x * 0.5)
        // update y velocity based on x velocity (higher x = lower y)
        lui     t0, 0x3d80                  // ~
        mtc1    t0, f0                      // f0 = 0.0625, used to be 3f00 or 0.375
        mul.s   f0, f0, f2                  // ~
        sub.s   f4, f4, f0                  // f4 = AIR_Y_SPEED - (x velocity * 0.375)
		
	// this seems to apply movement only once
    ...
    // @ Description
    // Subroutine which handles Game and Watch's horizontal control for up special.
    ...
    // @ Description
    // Subroutine which handles collision for Game and Watch's up special.
    // Copy of subroutine 0x80156358, which is the collision subroutine for Mario's up special.
    // Loads the appropriate landing fsm value for Game and Watch.
    scope collision_: {
    ...
    scope goto_next: {
        ...
        lli     a1, GameAndWatch.Action.USP_Float    // a1(action id)
        or      a2, r0, r0                  // a2(starting frame) = 0
    ...
    scope float_interrupt_: {
        ...
        bnez    at, _end // cannot interrupt until frame 0
        nop
...
scope GameAndWatchDSP {
    ...
    scope ground_initial_: {
        ...
        lli     a1, GameAndWatch.Action.JudgeBegin // a1(action id) = DSP_Ground
        or      a2, r0, r0                  // a2(starting frame) = 0
        lui     a3, 0x3F80                  // a3(frame speed multiplier) = 1.0
        jal     0x800E6F24                  // change action
        sw      r0, 0x0010(sp)              // argument 4 = 0
    ...
    scope air_initial_: {
        ...
        sw      ra, 0x001C(sp)              // ~
        sw      a0, 0x0020(sp)              // store a0, ra
        lli     a1, GameAndWatch.Action.JudgeBeginAir    // a1(action id) = DSP_Air
        or      a2, r0, r0                  // a2(starting frame) = 0
        lui     a3, 0x3F80                  // a3(frame speed multiplier) = 1.0
        jal     0x800E6F24                  // change action
        sw      r0, 0x0010(sp)              // argument 4 = 0
	...
	_judge_1:
	lli     a1, GameAndWatch.Action.DSP_Ground_1          // a1(action id) = DSP
	lw      a0, 0x0018(sp)
	...
	nop
	lli     a1, GameAndWatch.Action.DSP_Air_1             // a1(action id) = DSP
	beq	r0, r0, _change_action			      // if aerial, change to aerial 1 state
	nop

	_judge_2:
	lli     a1, GameAndWatch.Action.DSP_Ground_2
	lw      a0, 0x0018(sp)
	lw      t1, 0x0084(a0)
	lw      t0, 0x0014C(t1)
	beqz	t0, _change_action
	nop
	lli     a1, GameAndWatch.Action.DSP_Air_2
	beq	r0, r0, _change_action
	nop

	_judge_3:
	lli     a1, GameAndWatch.Action.DSP_Ground_3
	lw      a0, 0x0018(sp)
	lw      t1, 0x0084(a0)
	lw      t0, 0x0014C(t1)
	beqz	t0, _change_action
	nop
	lli     a1, GameAndWatch.Action.DSP_Air_3
	beq	r0, r0, _change_action
	nop

	_judge_4:
	lli     a1, GameAndWatch.Action.DSP_Ground_4
	lw      a0, 0x0018(sp)
	lw      t1, 0x0084(a0)
	lw      t0, 0x0014C(t1)
	beqz	t0, _change_action
	nop
	lli     a1, GameAndWatch.Action.DSP_Air_4
	beq	r0, r0, _change_action
	nop

	_judge_5:
	lli     a1, GameAndWatch.Action.DSP_Ground_5
	lw      a0, 0x0018(sp)
	lw      t1, 0x0084(a0)
	lw      t0, 0x0014C(t1)
	beqz	t0, _change_action
	nop
	lli     a1, GameAndWatch.Action.DSP_Air_5
	beq	r0, r0, _change_action
	nop

	_judge_6:
	lli     a1, GameAndWatch.Action.DSP_Ground_6
	lw      a0, 0x0018(sp)
	lw      t1, 0x0084(a0)
	lw      t0, 0x0014C(t1)
	beqz	t0, _change_action
	nop
	lli     a1, GameAndWatch.Action.DSP_Air_6
	beq	r0, r0, _change_action
	nop

	_judge_7:
	lli     a1, GameAndWatch.Action.DSP_Ground_7
	lw      a0, 0x0018(sp)
	lw      t1, 0x0084(a0)
	lw      t0, 0x0014C(t1)
	beqz	t0, _change_action
	nop
	lli     a1, GameAndWatch.Action.DSP_Air_7
	beq	r0, r0, _change_action
	nop

	_judge_8:
	lli     a1, GameAndWatch.Action.DSP_Ground_8
	lw      a0, 0x0018(sp)
	lw      t1, 0x0084(a0)
	lw      t0, 0x0014C(t1)
	beqz	t0, _change_action
	nop
	lli     a1, GameAndWatch.Action.DSP_Air_8
	beq	r0, r0, _change_action
	nop

	_judge_9:
	lli     a1, GameAndWatch.Action.DSP_Ground_9
	lw      a0, 0x0018(sp)
	lw      t1, 0x0084(a0)
	lw      t0, 0x0014C(t1)
	beqz	t0, _change_action
	nop
	lli     a1, GameAndWatch.Action.DSP_Air_9
	beq	r0, r0, _change_action
	nop
	...
	lli    t1, GameAndWatch.Action.DSP_Ground_1     // t1 = Action.DSP_Ground_1
        beq    t0, t1, _one_damage    					// branch to one damage
	nop
        lli    t1, GameAndWatch.Action.DSP_Air_1    	// t1 = Action.DSP_Air_1
        beq    t0, t1, _one_damage   					// branch to one damage
	nop

	lli    t1, GameAndWatch.Action.DSP_Ground_5     // t1 = Action.DSP_Ground_5
        beq    t0, t1, _five_hover   					// branch to five hover
	nop
        lli    t1, GameAndWatch.Action.DSP_Air_5    	// t1 = Action.DSP_Air_5
        beq    t0, t1, _five_hover   					// branch to five hover
	nop

	lli    t1, GameAndWatch.Action.DSP_Ground_7     // t1 = Action.DSP_Ground_7
        beq    t0, t1, _seven_food  					// branch to seven food
	nop
        lli    t1, GameAndWatch.Action.DSP_Air_7    	// t1 = Action.DSP_Air_7
        beq    t0, t1, _seven_food  					// branch to seven food
	nop
	...
	lw      a0, 0x08E8(v0)              // v0 = MRGAWTHREED joint (he doesn't have a 0x95C joint?)
	lw      t0, 0x0084(a2)
