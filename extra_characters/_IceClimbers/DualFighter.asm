    ...
    scope AI: {
        ...
        scope cpu_main: {
            ...
            _is_sister: {
                ...
                lw      t1, 0x0024(t5)
                lli     at, Action.Grab
                beq     at, t1, _grab_press_r_once
                lli     at, Action.GrabPull
                beq     at, t1, _set_direction
                ...
                // When Popo starts a grab, Nana will press R once for item drops 
                scope _grab_press_r_once: {
                    // check if brother is in the first frame of a grab
                    // if we're here we already know that the brother is in a grab state
                    lw at, 0x4(t5) // at = brother object
                    lw at, 0x78(at) // at = brother's current animation frame
                    mtc1 at, f2 // f2 = brother's current animation frame
                    lui at, 0x3F80 // at = 1.0F
                    mtc1 at, f4 // f4 = 1.0F

                    c.eq.s f2, f4 // if brother's animation frame == 1.0F
                    nop
                    bc1f _clear // if not, just clear the inputs
                    nop

                    // check if we're holding an item
                    lw      at, 0x84C(s0) // held item
                    beqz    at, _clear // if not holding an item, clear inputs
                    nop

                    // assign a grab press to our input buffer
                    lli     t1, Joypad.R // t1 = R press mask
                    lb      t2, 0x0002(t0) // t2 = joystick X
                    lb      t3, 0x0003(t0) // t3 = joystick Y
                    lw      t4, 0x0004(t0) // t4 = facing direction
