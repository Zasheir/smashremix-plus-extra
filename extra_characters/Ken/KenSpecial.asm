...
// This file contains subroutines used by Ken's special moves.
scope KenSpecial: {
    scope OnActionChanged: {
        ...
        scope _special_cancellable_action_check: {
            // a1 = new action id
            lli t0, Ken.Action.JAB_L
            beq a1, t0, _continue
            lli t0, Ken.Action.JAB_L2
            beq a1, t0, _continue
            lli t0, Ken.Action.JAB_CLOSE
            beq a1, t0, _continue
            lli t0, Ken.Action.UTILT_H
            beq a1, t0, _continue
            lli t0, Ken.Action.UTILT_L
            beq a1, t0, _continue
            lli t0, Ken.Action.DTILT_H
            beq a1, t0, _continue
            lli t0, Ken.Action.DTILT_L
            beq a1, t0, _continue
            lli t0, Ken.Action.FTILT_CLOSE
            beq a1, t0, _continue
        ...
        scope _auto_turnaround_check: {
            ...
            beq a1, t0, _continue
            lli t0, Ken.Action.COMMAND_KICK
            beq a1, t0, _continue
            nop
    ...
    Character.table_patch_start(on_action_changed, Character.id.KEN, 0x4)
    dw OnActionChanged
    OS.patch_end()
    ...
    scope SpecialCancel: {
        ...
        scope interrupt: {
            ...
            scope check_tilt_strength: {
                ...
                lli a1, Ken.Action.DTILT_L // a1 = action to change to

                lli t1, Action.UTilt // t1 = action to check against
                beq t0, t1, change_action // if current action matches, change action
                lli a1, Ken.Action.UTILT_L // a1 = action to change to

                lli t1, Action.FTilt // t1 = action to check against
                beq t0, t1, change_action // if current action matches, change action
                lli a1, Ken.Action.FTILT_L // a1 = action to change to

                lli t1, Action.Jab1 // t1 = action to check against
                beq t0, t1, change_action // if current action matches, change action
                lli a1, Ken.Action.JAB_L // a1 = action to change to

                lli t1, Ken.Action.JAB_CLOSE // t1 = action to check against
                beq t0, t1, change_action // if current action matches, change action
                lli a1, Ken.Action.JAB_L // a1 = action to change to
            ...
            scope check_jab_continue: {
                ...
                lli t1, Ken.Action.JAB_L // t1 = Action to check against
                beq t0, t1, change_action // if current action matches, change action
                lli a1, Ken.Action.JAB_L2 // action to change to

                lli t1, Ken.Action.JAB_L2 // t1 = Action to check against
                beq t0, t1, change_action // if current action matches, change action
                lli a1, Ken.Action.JAB_L3 // action to change to
        ...
        scope proximity_move_check: {
            ...
            addiu a1, r0, Ken.Action.JAB_CLOSE

            lli t2, Ken.Action.FTILT_L
            beq t1, t2, change_action
            addiu a1, r0, Ken.Action.FTILT_CLOSE
    ...
    scope USP: {
        ...
        fire_anim_struct_ken:
        dw 0x020F0000
        dw Character.KEN_file_7_ptr
        dw 0x501C0000 // C->A?
        ...
        dw 0x0000021C // customized because this animation was added to an existing file, these are offsets

        // @ Description
        // Subroutine which runs when Ken initiates an aerial up special.
        ...
        scope air_initial_: {
            ...
            sw r0, 0x0010(sp) // argument 4 = 0
            lli a1, Ken.Action.USP_AIR_L // a1 = Action.USPA
        ...
        // @ Description
        // Subroutine which runs when Ken initiates a grounded up special.
        // Changes action, and sets up initial variable values.
        scope ground_initial_: {
            ...
            _change_action:
            sw r0, 0x0010(sp) // argument 4 = 0
            lli a1, Ken.Action.USP_L // a1 = Action.USPG
            or a2, r0, r0 // a2 = float: 0.0
            jal 0x800E6F24 // change action
            lui a3, 0x3F80 // a3 = float: 1.0
            jal 0x800E0830 // unknown common subroutine
        ...
        // @ Description
        // Main subroutine for Ken's up special.
        // Based on subroutine 0x8015C750, which is the main subroutine of Fox's up special ending.
        // Modified to load Ken's landing FSM value and disable the interrupt flag.
        scope main_: {
            OS.routine_begin(0x20)
            ...
            lli t2, Ken.Action.USP_L
            beq t7, t2, light_to_hard_continue
            lli t2, Ken.Action.USP_AIR_L
            beq t7, t2, light_to_hard_continue
            nop
            b _main_normal
            nop
            ...
            lli a1, Ken.Action.USP_H // a1 = Action
            lli a1, Ken.Action.USP_AIR_H // a1 = Action
        ...
        scope goto_fall: {
            ...
            lw a0, 0x0020(sp) // a0 = player object
            lli a1, Ken.Action.USP_FALL // a1(action id)
        ...
        // @ Description
        // Subroutine which allows a direction change for Ken's up special.
        ...
        scope physics_: {
            ...
            lli t2, Ken.Action.USP_H
            beq t7, t2, _multiply_movement
            nop
            lui t0, 0x3FC0 // t0 = 1.5
            lli t2, Ken.Action.USP_AIR_H
            beq t7, t2, _multiply_movement
            nop
        ...
        // @ Description
        // Subroutine which handles Ken's horizontal control for up special.
        scope air_control_: {
        ...
        // @ Description
        // Collision wubroutine for Ken's up special.
        // Copy of subroutine 0x80156358, which is the collision subroutine for Mario's up special.
        // Loads the appropriate landing fsm value for Ken.
        scope collision_: {
            OS.save_registers()
    ...
    scope NSP: {
        ...
        scope ground_initial: {
            ...
            // in this case, we might wanna do a command kick
            // else, do NSP

            lli a1, 0xE4 // a1 = action = grounded nsp

            // check stick buffer
            lbu t0, 0x26A(a0) // t0 = horizontal stick hold buffer
            slti t0, t0, 8
            beqz t0, change_action
            nop

            fsmash_b:
            lli a1, Ken.Action.ROUNDHOUSE // a1 = action
        ...
        // @ Description 
        // main subroutine for Ken's Blaster
        scope main: {
            addiu sp, sp, -0x0040
            sw ra, 0x0014(sp)
            swc1 f6, 0x003C(sp)
            ...
            if {defined Character.CHARACTER_ADDED_KEN} {
                // Check if we're Ken
            ...
            // Hit FGM
            lli at, Ken.FGM.PUNCH_M // at = KEN_HIT_M
            ...
            // Hit FGM
            lli at, Ken.FGM.PUNCH_L // at = KEN_HIT_L
            sh at, 0x0146(v1) // save
            ...
            lw t0, 0x0008(v0)
            addiu t7, r0, Character.id.KEN
            bnel t0, t7, _standard
            lui t7, 0x3F80 // load normal reflect multiplier if not ken and thereby top speed of ken projectile will not increase
            ...
            Projectile.add_projectile(KEN_HADOUKEN)
            Projectile.set_projectile_hitlag_multiplier(Projectile.id.KEN_HADOUKEN, 0x3FC0) // 1.5x hitlag
            
            _blaster_projectile_struct:
            dw 0x00000000 // this has some sort of bit flag to tell it to use secondary type display list?
            dw Projectile.id.KEN_HADOUKEN
            ...
            dw Character.KEN_file_6_ptr // 0x0024 projectile data pointer
    ...
    scope DSP: {
        scope ground_initial: {
            ...
            dsmash_b:
            sw r0, 0x0048(a0) // set zero x speed
            lli a1, Ken.Action.COMMAND_KICK // a1 = action
        ...
        scope Start {
            ...
            scope goto_next: {
                ...
                b _continue
                lli a1, Ken.Action.DSP_H // a1(action id)

                _ground_weak:
                lli at, 0x1
                sw at, 0x017C(s0) // temp variable 1 (spins) = 1
                b _continue
                lli a1, Ken.Action.DSP_L // a1(action id)

                _aerial:
                lli at, 0x3
                sw at, 0x017C(s0) // temp variable 1 (spins) = 3
                b _continue
                lli a1, Ken.Action.DSP_AIR // a1(action id)
        ...
        scope Loop {
            ...
            scope goto_end: {
                ...
                _grounded:
                b _continue
                lli a1, Ken.Action.DSP_END // a1(action id)

                _aerial:
                b _continue
                lli a1, Ken.Action.DSP_AIREND // a1(action id)
    ...
    scope CommandKick: {
        scope interrupt: {
            ...
            lhu t0, 0x1BC(a1) // load button hold buffer

            andi t0, t0, Joypad.B // t0 = 0x40 if (B_PRESSED); else t0 = 0
            beq t0, r0, _end // skip if (!B_PRESSED)
            or v0, r0, r0 // return false (no action interrupt)

            lwc1 f2, 0x78(a0) // load current frame into f2

            lw t0, 0x0024(a1) // t0 = current action
            lli t1, Ken.Action.ROUNDHOUSE
            beq t0, t1, pc()+0xC
            lui at, 0x4120 // at = 10.0 roundhouse cancel frame
            lui at, 0x4140 // at = 12.0 command kick cancel frame

            mtc1 at, f4 // f4 = cancel frame
            c.eq.s f2, f4 // f2 == f4 ?
            nop
            bc1fl _end // skip if frame != at
            or v0, r0, r0 // return false (no action interrupt)
            ...
            lli a1, Ken.Action.COMMAND_KICK_2 // a1 = action ID
            lui a2, 0x40E0 // a2 = starting frame = 7.0
            lui a3, 0x3F80 // a3(frame speed multiplier) = 1.0
            sw r0, 0x0010(sp) // argument 4 = 0
            jal 0x800E6F24 // change action
            nop
            ...
            lli v0, 0x1 // return true (action interrupt)
