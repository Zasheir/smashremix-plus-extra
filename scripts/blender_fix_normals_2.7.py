...
for room in rigged_rooms:
    ...
    dup_room = bpy.context.scene.objects.active

    dup_room.modifiers.clear() # hope you didn't have an important modifier!
    regular_rooms.append(dup_room)
...
for idx, reg_room in enumerate(regular_rooms):
    ...
    reg_room.select = True
    bpy.context.scene.objects.active = rig_room
    bpy.ops.object.data_transfer(data_type='CUSTOM_NORMAL', loop_mapping='TOPOLOGY')

    reg_room.select = False

    # transfer normals from regular room to rigged room
    rig_room.select = True
    bpy.context.scene.objects.active = reg_room
    bpy.ops.object.data_transfer(data_type='CUSTOM_NORMAL', loop_mapping='TOPOLOGY')
